...
function loadParamsFromFile(jsonFile) {
  ...
  params["init_params"].push(
    BigInt(named_params.arithmetization_params.length)
  );
  for (let i in named_params.arithmetization_params) {
    params["init_params"].push(
      BigInt(named_params.arithmetization_params[i].value)
    );
  }
...
function loadPublicInput(public_input_path) {
  ...
  if (fs.existsSync(public_input_path)) {
    let public_input = losslessJSON.parse(
      fs.readFileSync(public_input_path, "utf8")
    );
    let result = [];
    for (let i in public_input) {
...
task("verify-circuit-proof-all").setAction(async (hre) => {
  ...
  for (const k in circuits) {
    ...
    const lookup_argument_contract = await ethers.getContract(
      "modular_lookup_argument_" + circuit
    );
    const gate_argument_contract = await ethers.getContract(
      "modular_gate_argument_" + circuit
    );
    const commitment_contract = await ethers.getContract(
      "modular_commitment_scheme_" + circuit
    );

    const verifier_contract = await ethers.getContract(
      "modular_verifier_" + circuit
    );
    ...
    await verifier_contract.verify(proof, public_input, {
      gasLimit: 30_500_000,
    });
    console.log("====================================");
