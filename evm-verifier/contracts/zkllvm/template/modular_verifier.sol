...
contract modular_verifier_template is IModularVerifier {
    ...
    bytes32 constant vk1 = bytes32(0x6be7eedaf38d6c27a998ad3ac2b59fb6272593ba1939939103d330ba365558bb);
    bytes32 constant vk2 = bytes32(0x9367cbb52e5a67071a0579a11baa6e3106ad399eb64781212a70868afe95a558);
    ...
    uint64 constant rows_amount = 65536;
    ...
    function initialize(
        ...
        tr_state.current_challenge = modular_commitment_scheme_template.initialize(tr_state.current_challenge);
    ...
    function public_input_direct(bytes calldata blob, uint256[] calldata public_input, verifier_state memory state)
        internal
        view
        returns (bool check)
    {
        ...
        for (uint256 i = 0; i < public_input.length;) {
            if (public_input[i] != 0) {
                uint256 L =
                    mulmod(Omega, field.inverse_static(addmod(state.xi, modulus - Omega, modulus), modulus), modulus);

                result = addmod(result, mulmod(public_input[i], L, modulus), modulus);
            }
        ...
        result = mulmod(result, addmod(field.pow_small(state.xi, rows_amount, modulus), modulus - 1, modulus), modulus);
        result = mulmod(result, field.inverse_static(rows_amount, modulus), modulus);

        // Input is proof_map.eval_proof_combined_value_offset
        if (result != basic_marshalling.get_uint256_be(blob, 3040)) check = false;
    }

    function verify(bytes calldata blob, uint256[] calldata public_input) public view returns (bool result) {
...

        {
            ...
            uint256[3] memory permutation_argument = modular_permutation_argument_template.verify(
                blob[0xa1:3041 + 3136],
...

        {
            ...
            state.F[7] = modular_gate_argument.verify(
                blob[table_offset:table_end_offset], transcript.get_field_challenge(tr_state, modulus)
            );
            state.F[7] = mulmod(
                state.F[7],
                addmod(
        ...
        uint256 F_consolidated;
        {
            ...
            for (uint8 i = 0; i < f_parts;) {
                F_consolidated = addmod(
                    F_consolidated,
                    mulmod(state.F[i], transcript.get_field_challenge(tr_state, modulus), modulus),
                    modulus
                );
                unchecked {
                    i++;
        ...
        //8. Commitment scheme verify_eval
        {
            ...
            if (
                !modular_commitment_scheme_template.verify_eval(
        ...
        //9. Final check
        {
            ...
            for (
                uint64 i = 0;
                i < uint64(uint8(blob[z_offset + basic_marshalling.get_length(blob, z_offset - 0x8) * 0x20 + 0xf]));
            ) {
                T_consolidated = addmod(
                    T_consolidated,
                    mulmod(
                        basic_marshalling.get_uint256_be(blob, table_offset + quotient_offset + i * 0x20),
                        factor,
                        modulus
                    ),
                    modulus
            ...
            if (state.b) console.log("SUCCESS!");
